...
public class Validator {
  ...
  public static void validateZone(XZone value, Collection<XZone> existingZones) {
    ...
    if(record.getToken() != null && !record.getToken().isEmpty()) {
      validateRecordTokenConflicts(ve, existingZones, record.getToken());
    }
  ...
  public static void validateRecordUpdate(XRecord record, XRecord targetRecord, Collection<XZone> existingZones) {
    ...
    if(record.getToken() != null && !record.getToken().isEmpty() && !record.getToken().equals(targetRecord.getToken())) {
      validateRecordTokenConflicts(ve, existingZones, record.getToken());
    }
    ve.throwOnDetails();
  }
    ...
    if(XType.CNAME.equals(record.getType())) {
      ...
      if(targetZone.getRecords().stream().anyMatch(x -> x.getName().equals(record.getName()) && XType.CNAME.equals(x.getType()))) {
        ve.withDetail(new ErrorDetail(ErrorMsg.VAL_RECORD_NAME_CONFLICT.name()));
      }
    }
	}

	private static void validateRecordTokenConflicts(ValidationException ve, Collection<XZone> existingZones, String token) {
    if(existingZones.stream().map(XZone::getRecords).flatMap(List::stream).anyMatch(x -> token.equals(x.getToken()))) {
